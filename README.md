...
| 数据访问 | Spring Data JPA | 主业务模型与仓储；复杂统计可用原生 SQL |
| 关系数据库 | MySQL 8.4 | 存储用户、会话、消息、知识库、Agent 规划数据 |
| 向量索引 | 自研本地扁平索引（Faiss IndexFlat 风格） | 精确余弦 + JSON 落盘；**非 Faiss 原生库**；无需独立向量服务 |
...
#### 向量索引：自研本地扁平索引（Faiss 风格）

实现类：`VectorIndexService`。

1. **选型**：题目 FAQ 允许 Chroma / Faiss 本地文件模式且不强制独立服务。本项目实现为 **自研扁平向量索引**（算法对齐 Faiss `IndexFlatIP` 的精确余弦），**未引入 Faiss / Chroma 原生依赖**，免 Docker。
2. **实现**：向量与 `kbId` / `serviceCode` / `priority` 等 metadata 写入 `FAISS_INDEX_DIR`（配置名沿用，默认 `backend/data/faiss-index/index.json`）。
3. **生产向能力**：删除按 `vectorId` 同步；同内容 upsert 跳过重新 Embedding；重启从磁盘加载；知识库/服务码过滤。语料更大时可再换 HNSW/IVF 或接真 Faiss，接口可保持不变。
4. **不依赖 Qdrant**：不要求 Docker 向量库即可完整跑通 RAG / Agent。
...
5. 生成后仅校验「数字+业务单位」（天/元/%/次等）；失败则 SSE `answer_replace` 整段替换为证据摘要（`answerStatus=degraded`）
...
2. **流式输出稳定性**  
   ...
   - SSE 使用命名事件（`message_start` / `token` / `answer_replace` / `citation` / `message_end` / `error`），前端按帧解析；校验失败时用 `answer_replace` 覆盖草稿，避免叠字。  
...
- **建表**：`backend/sql/schema.sql` 为完整 DDL（12 张表，已合并 `migrate_*.sql` 字段）。新库执行该脚本即可；`migrate_*.sql` 仅供旧库升级。  
- **初始数据**：不在 SQL 中 INSERT，由启动时 `DataSeeder` 写入：

1. 预置客服库三篇文档（classpath `seed/`）：`公司产品介绍.txt`、`常见问题FAQ.md`、`退换货政策.txt`（合计约 2000–5000 字量级，便于测检索）。  
2. 预置技术文档库若干短篇，供 Agent 使用。  
3. 切块后写入 MySQL，并 upsert 到本地向量索引（`data/faiss-index`），启动后即可直接问答；再次启动时同内容可复用已落盘向量。  
4. 若库中已有用户，则跳过重复种子，仅对已有文档重建/对齐向量索引。  

详见 `backend/README.md`「数据库初始化」。
