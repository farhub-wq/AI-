...
本地扁平向量索引记录与 chunk 对齐（JSON 落盘，非 Faiss 原生库），典型字段：
...
完整建表脚本：`backend/sql/schema.sql`（12 张表，与 JPA 实体对齐，已合并 `migrate_*.sql`）。  
**初始数据**：不写在 SQL 里，由 `DataSeeder` 在空库首次启动时写入（见 `backend/README.md`）。
...
3. Agent 用目录 + 依赖边 + `plan_json` + **错误记忆表**，完整回答「改谁 / 并行 / 串行」，并支持反思重试与自我修正。
