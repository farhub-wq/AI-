...
邮箱注册与手机号注册分开：`email` / `phone` 均可为空（二选一必填），各自唯一；昵称 `display_name` 亦全局唯一；邮箱后缀由业务白名单校验（qq/163/gmail 等）；`password_hash` 存 BCrypt。注册接口对同一账号+正确密码具备幂等性。
