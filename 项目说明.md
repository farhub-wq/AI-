...
**主路径（`planningMode=multi_agent_llm`）**

1. 技术库检索 + 服务目录/依赖表作为 Tool 上下文接地  
2. **ImpactAgent**（LLM）：判断改哪些微服务；注入 `agent_error_memory` 历史教训  
3. **ReflectionAgent**（层级监督）：评估 Impact；否决则落库错误并指定重试  
4. **DagAgent**（LLM）：任务 DAG / 并行组 / 发布顺序；程序校验 + 反思可再试  
5. **ReviewAgent**（LLM）：验收步骤与评审清单  
6. **ReflectionAgent@final**：整体评估，可指定 Impact/Dag/Review 重试  
7. 落库 `agent_plans`（含 `agentTrace`）→ 工作台展示  

**降级路径（`planningMode=rules_fallback`）**

1. `parseSignals` → 2. 检索打分 → 3. 依赖表 `buildTasks` → 4. 并行组/校验 → 5. 可选文案润色 → 6. 落库  
...
| 改造项 | 做法 | 对现有功能的影响 |
| --- | --- | --- |
| 增强服务识别准确率 | 丰富技术库与依赖表（Tool 接地材料） | 无影响客服问答 |
| 更通用的依赖推断 | 减少硬编码服务名分支 | 仅影响规划页输出 |
| LLM 多 Agent 规划 | ✅ `AgentMultiAgentPipeline` + **ReflectionAgent**；错误写入 `agent_error_memory` 自我修正；失败降级规则 | 客服链路不变 |
...
本项目做成系统内建能力：由 **LLM Agent** 在文档/目录/依赖表接地后回答「改谁 / 谁先谁后 / 谁能一起做」；规则引擎仅作降级兜底；并用 `partial/failed` 暴露证据不足。

### 4.7 效果说明（如何验证拆解质量）

| 用例 | 期望 | 判定 |
| --- | --- | --- |
| 下单后发短信 | 标出订单/用户/通知/前端及依赖边 | 工作台 `impactedServices` + DAG + 发布顺序 |
| 模糊需求 | 暴露证据不足 | `missingEvidence` / `partial` |
| LLM 不可用 | 规则降级仍可评审 | `planningMode=rules_fallback` |

更完整对照表见 `docs/AI架构设计.md` §5.4–§5.5。
...
3. 切块后写入 MySQL，并 upsert 到本地扁平向量索引（`data/faiss-index`），启动后即可直接问答；再次启动时同内容可复用已落盘向量。  
