...
> 对应 `Read.md` 加分项 6：当 Agent 收到需求（如「用户下单后自动发送短信通知」）与技术/接口文档后，如何判断——**改哪几个微服务？哪些可同时进行？哪些必须按先后顺序？**  
> 要求可用流程图或示例说明；不要求完整写代码，**项目内实现更好**。
...
> 题目给定需求 + 本项目种子技术文档/服务依赖，运行「生成变更规划」后的典型结果。
